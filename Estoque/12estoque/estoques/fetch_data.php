...
if($sgbd == 'mysql'){
    $results = $pdo->prepare("SELECT id,compra_id,quantidade,valor_unitario FROM estoques ORDER BY id LIMIT $start, $row_limit");
}elseif($sgbd == 'pgsql'){
    $results = $pdo->prepare("SELECT id,compra_id,quantidade,valor_unitario FROM estoques ORDER BY id LIMIT $row_limit OFFSET $start");    
}
