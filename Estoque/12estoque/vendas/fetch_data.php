...
if($sgbd == 'mysql'){
    $results = $pdo->prepare("SELECT id,produto_id,quantidade,data,valor_unitario FROM vendas ORDER BY id LIMIT $start, $row_limit");
}elseif($sgbd == 'pgsql'){
    $results = $pdo->prepare("SELECT id,produto_id,quantidade,data,valor_unitario FROM vendas ORDER BY id LIMIT $row_limit OFFSET $start");    
}
